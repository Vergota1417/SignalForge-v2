...
import { getEvidenceEvaluation } from './evaluation.js';
import { getOptimizationReport } from './optimization-report.js';
import { getOperationsStatus, recordOperation } from './operations.js';
import { getDetectionLatencyAudit } from './detection-latency.js';
import { broadDiscoveryCoverage, isBroadDiscoverySlot } from './scanner-schedule.js';
...
export default {
  async fetch(request,env){
    try{
      ...
      if(url.pathname==='/api/simulation/start'&&request.method==='POST'){const body=await readJson(request);if(!await portfolioAuthorized(request,env,body))return json({error:'Simulation capital changes require an authorized SignalForge phone.'},403);return json({simulation:await setStartingCapital(env,body?.amount)});}
      if(url.pathname==='/api/simulation/contribution'&&request.method==='POST'){const body=await readJson(request);if(!await portfolioAuthorized(request,env,body))return json({error:'Simulation contributions require an authorized SignalForge phone.'},403);return json({simulation:await addSimulationContribution(env,body?.amount,{note:String(body?.note||'')})});}
      if(request.method!=='GET')return json({error:'Method not allowed.'},405);

      if(url.pathname==='/api/health')return json({ok:true,service:'SignalForge-v2',marketDataConfigured:Boolean(env.TWELVE_DATA_API_KEY),databaseConfigured:Boolean(env.DB),watchlist:watchlist(env),weeklyInvestmentEngine:true,dynamicDiscovery:true,discoveryPoolSize:120,weeklyResearchTarget:36,intradayTimingOnly:true,symbolSearch:true,opportunityRadar:true,smartMarketScreener:true,screenerPromotion:true,adaptivePriorityExecution:true,priorityPricePulseMinutes:5,nearReadyExecutionMinutes:15,detectionLatencyAudit:true,broadDiscoveryCoverage:broadDiscoveryCoverage(),afterHoursResearch:true,weekendResearch:true,dailyBackgroundSummary:true,portfolioStrategy:true,paperSimulation:true,simulationContributions:true,calibratedScoring:true,evidenceEvaluation:true,evidenceOptimization:true,championChallenger:true,fractionalSizing:true,pushConfigured:pushConfigured(env),pushSubscribers:await countPushSubscriptions(env),pushTest:true});
      if(url.pathname==='/api/operations-status')return json({operations:await getOperationsStatus(env)});
      if(url.pathname==='/api/detection-latency'){const symbol=sanitizeSymbol(url.searchParams.get('symbol'));if(!symbol)return json({error:'Valid symbol is required.'},400);return json({audit:await getDetectionLatencyAudit(env,{symbol,lookbackDays:clampInt(url.searchParams.get('days'),1,30,3)})});}
      if(url.pathname==='/api/evidence-evaluation')return json({evaluation:await getEvidenceEvaluation(env,{horizon:normalizeEvaluationHorizon(url.searchParams.get('horizon')),minSample:clampInt(url.searchParams.get('minSample'),3,100,20)})});
      if(url.pathname==='/api/evidence-optimization')return json({optimization:await getOptimizationReport(env,{horizon:normalizeEvaluationHorizon(url.searchParams.get('horizon')),minSample:clampInt(url.searchParams.get('minSample'),5,100,20),winnerThreshold:clampNum(url.searchParams.get('winnerThreshold'),.01,.25,.05),challenger:{statuses:['BUY NOW'],minReadiness:clampNum(url.searchParams.get('minReadiness'),0,100,80),minRvol:clampNum(url.searchParams.get('minRvol'),0,10,1.5),minGates:clampInt(url.searchParams.get('minGates'),0,4,4),requireNormalRegime:url.searchParams.get('normalRegime')!=='0',requireStrongSector:url.searchParams.get('strongSector')!=='0'}})});
      ...
      if(url.pathname==='/api/market-data'){
        ...
        if(url.searchParams.get('cacheOnly')==='1'){const market=await getCachedMarket(env,symbol,timeframe,30*86_400_000);if(!market)return json({error:'Cached detail unavailable.',cacheMiss:true,symbol,timeframe},404);return json({symbol,timeframe,candles:market.candles,source:market.source,cached:true,fetchedAt:market.fetchedAt,cacheOnly:true});}
...
async function runScheduledCycle(env,scheduledTime){
  ...
  try{
    await ensureSchema(env);const p=easternParts(now),minutes=Number(p.hour)*60+Number(p.minute),weekday=p.weekday;
    await recordOperation(env,'cron-heartbeat',{status:'OK',at:now.getTime(),detail:{weekday,minutes,marketDataConfigured:Boolean(env.TWELVE_DATA_API_KEY)}});
    if(!env.TWELVE_DATA_API_KEY)return;
    if(weekday==='Sat'||weekday==='Sun'){if(minutes!==675)return;let weekendResearch=null;if(weekday==='Sat'){weekendResearch=await runAfterHoursResearch(env,{now:now.getTime(),maxPerRun:6,expandUniverse:true});console.log(JSON.stringify({event:'weekend_research_cycle',weekday,...weekendResearch}));}const[screener,research]=await Promise.all([getSmartScreenerSnapshot(env,{limit:10}),getAfterHoursResearchStatus(env)]),top=bestSummaryCandidate(screener?.rows||[]),push=await broadcastBackgroundSummaryPush(env,{dayLabel:weekday==='Sat'?'Saturday':'Sunday',top,research,weekend:true,occurredAt:now.getTime()});console.log(JSON.stringify({event:'weekend_background_summary',weekday,symbol:top?.symbol||null,researchRan:Boolean(weekendResearch),...push}));return;}
    if(weekday==='Fri'&&minutes>=840&&minutes<=930&&minutes%15===0){const result=await runWeeklyResearchBatch(env,{batchSize:6,now});console.log(JSON.stringify({event:'weekly_research_batch',weekKey:result.weekKey,scanned:result.scanned,cursor:result.cursor,universeSize:result.universeSize,completed:result.completed}));if(result.completed&&result.scanned.length){const[snapshot,positions]=await Promise.all([getWeeklyStrategySnapshot(env),listPortfolioPositions(env)]),top=rankOpportunities(snapshot.ranked,positions)[0];if(top){try{const push=await broadcastWeeklyOpportunityPush(env,{weekKey:snapshot.weekKey,row:top,occurredAt:Date.now()});console.log(JSON.stringify({event:'weekly_opportunity_push',symbol:top.symbol,state:top.strategy?.state,...push}));}catch(error){console.error(JSON.stringify({event:'weekly_opportunity_push_error',message:error?.message||String(error)}));}}}return;}
    if(isBroadDiscoverySlot(weekday,minutes)){const radar=await runRadarDiscovery(env,{batchSize:5,now:now.getTime()}),promotion=await runScreenerPromotion(env,{maxPromotions:1,now:now.getTime()});console.log(JSON.stringify({event:'discovery_promotion_cycle',scanned:radar.scanned.map(x=>x.symbol),tiers:radar.tiers,selected:radar.selected,leaders:radar.leaders.map(x=>x.symbol),promoted:promotion.promoted,candidates:promotion.candidates,cursor:radar.cursor,universeSize:radar.universeSize}));return;}
    const priorityExecutionSlot=minutes>=590&&minutes<=955&&minutes%5===0&&minutes%15!==0;if(priorityExecutionSlot){const pulse=await runPriorityExecutionPulse(env,{maxCandidates:2,now:now.getTime()});await recordOperation(env,'priority-execution',{status:'OK',at:now.getTime(),detail:{candidates:pulse.candidates||[],pulsed:(pulse.pulsed||[]).map(row=>({symbol:row.symbol,status:row.status,gatesReady:row.gatesReady,rr:row.rr})),skipped:pulse.skipped||null}});console.log(JSON.stringify({event:'priority_execution_cycle',...pulse}));return;}
...
function normalizeEvaluationHorizon(v){const n=Number(v);return[1,3,5,10,20].includes(n)?n:10;}
