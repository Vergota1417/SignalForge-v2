...
async function runScheduledCycle(env,scheduledTime){
  try{
    await ensureSchema(env);const now=new Date(scheduledTime||Date.now());if(!env.TWELVE_DATA_API_KEY)return;const p=easternParts(now),minutes=Number(p.hour)*60+Number(p.minute),weekday=p.weekday;
