...
export async function runPaperSimulation(env,{now=Date.now()}={}){
  ...
  if(lastId!==state.lastSignalEventId){
    await env.DB.prepare(`UPDATE paper_sim_state SET last_signal_event_id=?,updated_at=? WHERE id=1`).bind(lastId,now).run();
  }
...
async function processEvent(env,event){
  ...
  if(status==='BUY NOW'&&!open){
    await openPosition(env,{event,analysis});
  }else if(status==='SELL / EXIT'&&open){
    await closePosition(env,symbol,{signalPrice:Number(event.price),status,eventId:Number(event.id)||null,reason:'SignalForge changed to SELL / EXIT.',closedAt:Number(event.createdAt)||Date.now()});
  }
}

async function openPosition(env,{event,analysis}){
  const state=await getState(env);
  const signalPrice=positive(event.price);
  if(!(signalPrice>0)||!(state.cash>5))return;
  const stop=positive(analysis?.thesisBreak);
  const target=positive(analysis?.target);
  if(!(stop>0&&stop<signalPrice&&target>signalPrice))return;
  const fill=signalPrice*(1+SLIPPAGE_PCT);
  const riskPerShare=Math.max(.01,fill-stop);
  const equity=await currentEquity(env);
  const riskBudget=Math.max(0,equity*RISK_PER_TRADE);
  const maxAllocation=Math.max(0,equity*MAX_POSITION_PCT);
  const sharesByRisk=riskBudget/riskPerShare;
  const sharesByCap=Math.min(maxAllocation,state.cash)/fill;
  const shares=Math.max(0,Math.min(sharesByRisk,sharesByCap));
  if(!(shares>0.0001))return;
  const cost=shares*fill;
  const now=Number(event.createdAt)||Date.now();
  await env.DB.batch([
    env.DB.prepare(`INSERT INTO paper_positions(symbol,entry_price,shares,entry_signal_price,stop_price,target_price,opened_at,signal_event_id,entry_readiness,entry_status,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`)
      .bind(String(event.symbol),fill,shares,signalPrice,stop,target,now,Number(event.id)||0,Number(event.readiness)||0,String(event.status||'BUY NOW'),now),
...
async function reconcileOpenPositions(env,{now=Date.now()}={}){
  const [positions,signals]=await Promise.all([listPositions(env),listSignals(env)]);
  const signalMap=new Map(signals.map(row=>[row.symbol,row]));
  for(const position of positions){
    const signal=signalMap.get(position.symbol);
    const price=positive(signal?.price||signal?.analysis?.latest?.close);
    if(!(price>0))continue;
    if(signal?.status==='SELL / EXIT'||price<=position.stopPrice){
      await closePosition(env,position.symbol,{signalPrice:price,status:'SELL / EXIT',eventId:null,reason:price<=position.stopPrice?'Paper stop/thesis level reached.':'SignalForge changed to SELL / EXIT.',closedAt:now});
    }else if(price>=position.targetPrice){
      await closePosition(env,position.symbol,{signalPrice:price,status:'TARGET HIT',eventId:null,reason:'Paper structure target reached.',closedAt:now});
    }
  }
}

async function closePosition(env,symbol,{signalPrice,status,eventId,reason,closedAt}){
  const p=await env.DB.prepare(`SELECT symbol,entry_price AS entryPrice,shares,entry_status AS entryStatus,entry_readiness AS entryReadiness,opened_at AS openedAt,signal_event_id AS signalEventId FROM paper_positions WHERE symbol=?`).bind(symbol).first();
  if(!p)return;
  const market=positive(signalPrice);
  if(!(market>0))return;
  const exit=market*(1-SLIPPAGE_PCT);
  const proceeds=Number(p.shares)*exit;
  const pnl=(exit-Number(p.entryPrice))*Number(p.shares);
  const pnlPct=Number(p.entryPrice)>0?exit/Number(p.entryPrice)-1:0;
  const now=Number(closedAt)||Date.now();
  await env.DB.batch([
    env.DB.prepare(`INSERT INTO paper_trades(symbol,entry_price,exit_price,shares,pnl,pnl_pct,entry_status,exit_status,entry_readiness,opened_at,closed_at,entry_signal_event_id,exit_signal_event_id,exit_reason) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
      .bind(symbol,Number(p.entryPrice),exit,Number(p.shares),pnl,pnlPct,String(p.entryStatus),String(status),Number(p.entryReadiness)||0,Number(p.openedAt),now,Number(p.signalEventId),eventId,reason),
...
async function saveEquitySnapshot(env,now){
  const state=await getState(env);
  const positions=await listPositions(env);
  const signals=await listSignals(env);
  const signalMap=new Map(signals.map(row=>[row.symbol,row]));
  const positionsValue=positions.reduce((sum,p)=>sum+p.shares*positive(signalMap.get(p.symbol)?.price||p.entryPrice),0);
  const equity=state.cash+positionsValue;
  const last=await env.DB.prepare(`SELECT created_at AS createdAt FROM paper_equity_snapshots ORDER BY id DESC LIMIT 1`).first();
  if(!last||now-Number(last.createdAt)>=15*60*1000){
    await env.DB.prepare(`INSERT INTO paper_equity_snapshots(equity,cash,positions_value,created_at) VALUES(?,?,?,?)`).bind(equity,state.cash,positionsValue,now).run();
  }
}

export async function getSimulationSnapshot(env){
  await ensureSimulationSchema(env);
  const [state,positions,trades,signals,equityRows]=await Promise.all([
    getState(env),listPositions(env),listTrades(env,100),listSignals(env),
    env.DB.prepare(`SELECT equity,created_at AS createdAt FROM paper_equity_snapshots ORDER BY id ASC LIMIT 2000`).all()
  ]);
  const signalMap=new Map(signals.map(row=>[row.symbol,row]));
  const open=positions.map(p=>{
    const mark=positive(signalMap.get(p.symbol)?.price||p.entryPrice);
    const marketValue=p.shares*mark;
    const unrealized=(mark-p.entryPrice)*p.shares;
    return {...p,mark,marketValue,unrealized,unrealizedPct:p.entryPrice>0?mark/p.entryPrice-1:0,currentStatus:signalMap.get(p.symbol)?.status||'UNKNOWN'};
  });
  const positionsValue=open.reduce((sum,p)=>sum+p.marketValue,0);
  const equity=state.cash+positionsValue;
  const wins=trades.filter(t=>t.pnl>0),losses=trades.filter(t=>t.pnl<=0);
  const grossWins=wins.reduce((s,t)=>s+t.pnl,0),grossLoss=Math.abs(losses.reduce((s,t)=>s+t.pnl,0));
  const realized=trades.reduce((s,t)=>s+t.pnl,0),unrealized=open.reduce((s,p)=>s+p.unrealized,0);
  const curve=(equityRows.results||[]).map(r=>({equity:Number(r.equity)||0,createdAt:Number(r.createdAt)||0}));
  ...
  return {
    mode:'FORWARD PAPER TEST',startedAt:state.startedAt,startingCash:state.startingCash,cash:state.cash,equity,positionsValue,
    realizedPnl:realized,unrealizedPnl:unrealized,totalPnl:equity-state.startingCash,returnPct:state.startingCash>0?equity/state.startingCash-1:0,
    openPositions:open,closedTrades:trades,totalTrades:trades.length,wins:wins.length,losses:losses.length,winRate:trades.length?wins.length/trades.length:0,
    profitFactor:grossLoss>0?grossWins/grossLoss:(grossWins>0?null:0),maxDrawdown,curve,
    assumptions:{riskPerTrade:RISK_PER_TRADE,maxPositionPct:MAX_POSITION_PCT,slippagePct:SLIPPAGE_PCT,commission:0,entryRule:'Open only on a new BUY NOW signal.',exitRule:'Exit on SELL / EXIT, fixed thesis stop, or fixed structure target.',lookAhead:false}
  };
}
