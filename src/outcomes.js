...
export async function runOutcomeTracker(env,{now=Date.now(),maxSymbols=4,observationType=null,requiredHorizon=20}={}){
  try{
    await ensureOutcomeSchema(env);const cutoff=now-12*60*60*1000,type=normalizeObservationType(observationType),completionHorizon=normalizeRequiredHorizon(requiredHorizon);
    const query=type
      ?env.DB.prepare(`SELECT e.id,e.symbol,e.observed_at AS observedAt,e.price,e.target,e.thesis_break AS thesisBreak,e.payload_json AS payloadJson FROM evidence_observations e WHERE e.observation_type=? AND e.observed_at<=? AND e.price>0 AND NOT EXISTS(SELECT 1 FROM evidence_outcomes o WHERE o.observation_id=e.id AND o.horizon_sessions=?) ORDER BY e.observed_at ASC,e.id ASC LIMIT ?`).bind(type,cutoff,completionHorizon,MAX_PENDING_ROWS)
      :env.DB.prepare(`SELECT e.id,e.symbol,e.observed_at AS observedAt,e.price,e.target,e.thesis_break AS thesisBreak,e.payload_json AS payloadJson FROM evidence_observations e WHERE e.observed_at<=? AND e.price>0 AND NOT EXISTS(SELECT 1 FROM evidence_outcomes o WHERE o.observation_id=e.id AND o.horizon_sessions=?) ORDER BY e.observed_at ASC,e.id ASC LIMIT ?`).bind(cutoff,completionHorizon,MAX_PENDING_ROWS);
    ...
    const result={observationType:type||'ALL',requiredHorizon:completionHorizon,symbolsProcessed:symbols.length,observationsConsidered:pending.length,outcomesCompleted:completed.length,deferred:deferred.length,errors,completed};
    await recordOperation(env,'outcome-tracker',{status:errors.length&&!completed.length?'ERROR':pending.length?'OK':'IDLE',at:now,detail:{observationType:result.observationType,requiredHorizon:result.requiredHorizon,symbolsProcessed:result.symbolsProcessed,observationsConsidered:result.observationsConsidered,outcomesCompleted:result.outcomesCompleted,deferred:result.deferred,errors:errors.slice(0,6)}});return result;
  }catch(error){await recordOperation(env,'outcome-tracker',{status:'ERROR',at:now,detail:{observationType:normalizeObservationType(observationType)||'ALL',requiredHorizon:normalizeRequiredHorizon(requiredHorizon),message:error?.message||String(error)}}).catch(()=>{});throw error;}
}
...
function normalizeRequiredHorizon(value){const horizon=Number(value);return OUTCOME_HORIZONS.includes(horizon)?horizon:20;}
