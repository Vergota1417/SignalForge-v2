...
async function ensureProviderUsageDetailSchema(env){
  ...
  if(!ready){
    ready=env.DB.prepare(`CREATE TABLE IF NOT EXISTS provider_usage_detail(day_key TEXT NOT NULL,purpose TEXT NOT NULL,requests INTEGER NOT NULL DEFAULT 0,updated_at INTEGER NOT NULL,PRIMARY KEY(day_key,purpose))`).run().catch(error=>{detailSchemaReadyByDb.delete(env.DB);throw error;});
