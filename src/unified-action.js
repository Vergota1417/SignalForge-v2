export function unifiedActionState({signal=null,earlyMovement=null}={}){
  ...
  const failed=Array.isArray(analysis?.criticalFailed)?analysis.criticalFailed:[],confirmation=analysis?.intradayConfirmation||null,hardAuthorized=analysis?.hardBuyGuardrails?.pass===true;
  ...
  const details={acceleration,readiness,gatesReady,gateTotal,failed,participation,hardAuthorized};
  ...
  if(status==='BUY NOW'&&hardAuthorized&&participation.pass)return result('BUY NOW','BUY WINDOW OPEN','Hard BUY authorization, higher-timeframe gates, and live participation/execution confirmation are aligned.',details);
  if(status==='BUY NOW'&&!hardAuthorized)return result('READY SOON','WAIT — HARD AUTHORIZATION REQUIRED','A saved BUY label is not executable until the current non-negotiable guardrails are verified.',details);
