...
export async function getExecutionTrace(env,{symbol,now=Date.now()}={}){
  ...
  const [radar,marketRows,signal,evidence,position,strategy,alert,outcomeRows]=await Promise.all([
    ...
    env.DB.prepare(`SELECT id,observation_type AS observationType,source,timeframe,status,readiness,observed_at AS observedAt,payload_json AS payloadJson FROM evidence_observations WHERE symbol=? AND observation_type='ANALYSIS' ORDER BY observed_at DESC,id DESC LIMIT 1`).bind(s).first().catch(()=>null),
    ...
    env.DB.prepare(`SELECT o.horizon_sessions AS horizonSessions,o.forward_return AS forwardReturn,o.first_hit AS firstHit,o.evaluated_at AS evaluatedAt FROM evidence_outcomes o JOIN evidence_observations e ON e.id=o.observation_id WHERE e.symbol=? AND e.observation_type='ANALYSIS' ORDER BY o.evaluated_at DESC,o.horizon_sessions`).bind(s).all().catch(()=>({results:[]}))
  ]);
  ...
  const discoveryState=quote?'COMPLETE':(signal||latestMarket)?'SKIPPED':'NOT_RUN';
  const stages=[
    stage('discovery',discoveryState,{updatedAt:Number(radar?.updatedAt)||0,score:num(quote?.rollingDiscoveryScore??quote?.discoveryScore??quote?.score),relativeVolume:num(quote?.relativeVolume),source:quote?.source||null,selectionSource:quote?'RADAR':'DIRECT_OR_MANUAL',reason:quote?'Saved radar discovery proof exists.':'This symbol was loaded/analyzed directly, so radar discovery was not required for this trace.'}),
  ...
  const completed=stages.filter(x=>x.state==='COMPLETE').length,skipped=stages.filter(x=>x.state==='SKIPPED').length,blocked=stages.filter(x=>x.state==='BLOCKED').length,warnings=stages.filter(x=>['WARNING','PENDING'].includes(x.state)).length;
  const traceAt=Math.max(...stages.map(x=>Number(x.updatedAt)||0),0),traceId=`SF-${s}-${traceAt||now}`;
  return{traceId,symbol:s,generatedAt:now,traceAt,observationalOnly:true,affectsBuyNow:false,summary:{completed,skipped,total:stages.length,blocked,warnings,finalStatus:signal?.status||analysis?.status||'NOT ANALYZED',provider:latestMarket?.source||null,lastDataAt:latestMarket?.fetchedAt||0,evidenceObserved:Boolean(evidence)},stages,evidencePayload:evidencePayload?{reason:evidencePayload.reason||'',criticalFailed:evidencePayload.criticalFailed||[]}:null};
}
