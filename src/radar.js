    ...
    for(const symbol of batch.symbols){
      try{
        ...
        if(isPermanentProviderSymbolError(error)){
          const quarantine=await quarantineDiscoverySymbol(env,symbol,{now,cooldownMs:PERMANENT_PROVIDER_RETIRE_MS,reason:'market-provider-symbol-permanently-rejected'}).catch(()=>null);
