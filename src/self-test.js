...
export async function runBackendSelfTest(env,{symbol='XOM',now=Date.now()}={}){
  ...
  await check('database',async()=>{await env.DB.prepare(`CREATE TABLE IF NOT EXISTS backend_self_test(id INTEGER PRIMARY KEY CHECK(id=1),checked_at INTEGER NOT NULL)`).run();await env.DB.prepare(`INSERT INTO backend_self_test(id,checked_at) VALUES(1,?) ON CONFLICT(id) DO UPDATE SET checked_at=excluded.checked_at`).bind(now).run();const row=await env.DB.prepare(`SELECT checked_at AS checkedAt FROM backend_self_test WHERE id=1`).first();if(Number(row?.checkedAt)!==Number(now))throw new Error('Database self-test write/read mismatch.');return{writeRead:true};});
  let market=null;
