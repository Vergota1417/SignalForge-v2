...
export function summarizeOpportunityValidation(episodes,{horizon=5,minSample=OPPORTUNITY_REVIEW_MIN_SAMPLE}={}){
  const normalizedHorizon=normalizeHorizon(horizon),completed=(episodes||[]).filter(row=>outcomeFor(row,normalizedHorizon)),pending=(episodes||[]).filter(row=>!outcomeFor(row,normalizedHorizon));
  const tiers=[
  ...
  const scoreGradientConfirmed=highScore.sampleSize>=Math.min(10,minSample)&&comparison.sampleSize>=10&&highScore.avgReturn>comparison.avgReturn&&highScore.winRate>=comparison.winRate;
  const criteria={sampleSize:highScore.sampleSize>=minSample,winRate:highScore.winRate>=.55,positiveExpectancy:highScore.avgReturn>0,positiveMarketExcess:highScore.avgMarketExcess>0,marketBeatRate:highScore.marketBeatRate>=.52,scoreGradient:scoreGradientConfirmed};
  const enough=highScore.sampleSize>=minSample,status=!enough?'COLLECTING':Object.values(criteria).every(Boolean)?'REVIEW CANDIDATE':'NOT VALIDATED';
  return{status,horizonSessions:normalizedHorizon,minSample:Number(minSample)||OPPORTUNITY_REVIEW_MIN_SAMPLE,totalEpisodes:(episodes||[]).length,completedEpisodes:completed.length,pendingEpisodes:pending.length,highScore,comparison,tiers,criteria,scoreGradientConfirmed,falsePositiveRate:highScore.sampleSize?1-highScore.winRate:null,marketUnderperformRate:highScore.sampleSize?1-highScore.marketBeatRate:null,reviewRule:'A REVIEW CANDIDATE requires 30+ completed 5-session high-score episodes, >=55% positive outcomes, positive average return, positive market excess, >=52% market-beat rate, and a better 80+ score cohort than the 60–79 cohort. Review status still cannot affect BUY NOW.',shadowOnly:true,affectsBuyNow:false};
}
