...
export function refreshIntervalFor(signal){const analysis=signal?.analysis||null,status=String(signal?.status||analysis?.status||'');if(analysis?.dailyGatesReady||status==='SETUP — READY SOON'||status==='WAIT FOR PULLBACK'||status==='BUY NOW')return NEAR_READY_RECHECK_MS;return PROMOTION_STALE_MS;}
