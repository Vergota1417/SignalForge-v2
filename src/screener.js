...
export async function runScreenerPromotion(env,{maxPromotions=2,now=Date.now()}={}){
  ...
  for(const candidate of candidates){
    try{
      ...
      const benchmarkEvidence=await loadBenchmarkEvidence(env,candidate.symbol,{stockCandles:market.candles,timeframe:'6M',purposePrefix:'promotion-benchmark-context',preloaded:{SPY:benchmarkCandles}});
      const event=await recordSignal(env,analysis);
      await recordAnalysisEvidence(env,analysis,{source:'screener-promotion',timeframe:'6M',quote:candidate,now,benchmarkContext:benchmarkEvidence.context});
      promoted.push({symbol:candidate.symbol,screenScore:candidate.screenScore,researchScore:candidate.research?.confirmationScore||0,status:analysis.status,readiness:analysis.readiness,changed:event.changed,benchmarkContext:benchmarkEvidence.context,benchmarkErrors:benchmarkEvidence.errors});
...
export function selectPromotionCandidates(quotes=[],signals=[],{owned=new Set(),now=Date.now(),limit=2,researchMap=new Map()}={}){
  ...
  return buildScreenerRows(quotes,signals,{researchMap}).filter(row=>!owned.has(row.symbol)).filter(row=>row.bucket!=='AVOID').filter(row=>row.discoveryScore>=20||row.relativeVolume>=1.25||row.scoreVelocity>=8||row.research?.confirmationScore>=60).filter(row=>{const signal=signalMap.get(row.symbol),updated=Number(signal?.updatedAt)||0;return !signal?.analysis||now-updated>=PROMOTION_STALE_MS;}).sort((a,b)=>promotionPriority(b)-promotionPriority(a)||b.screenScore-a.screenScore||b.scoreVelocity-a.scoreVelocity).slice(0,Math.max(1,Math.min(2,Number(limit)||2)));
}
