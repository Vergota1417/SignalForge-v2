...
export async function getMarketData(env,symbol,timeframe,forceRefresh=false,options={}){
  ...
  if(payload?.status==='error')throw twelveDataPayloadError(payload,'time series');
...
export async function searchSymbols(env,query){
  ...
  const payload=await fetchProviderJson(url,'Twelve Data symbol search');if(payload?.status==='error')throw twelveDataPayloadError(payload,'symbol search');
...
function twelveDataPayloadError(payload,context='request'){const message=String(payload?.message||payload?.code||'provider error'),status=Number(payload?.code)||400,kind=classifyTwelveDataFailure(status,message),error=new Error(`Twelve Data [${kind}]: ${message} (${context})`);error.status=status;error.provider='twelve-data';error.kind=kind;return error;}
function classifyTwelveDataFailure(status,message){const m=String(message||'').toLowerCase();if(status===401||status===403||m.includes('api key')||m.includes('authentication'))return'AUTH';if(status===429||m.includes('rate limit')||m.includes('credits'))return'RATE_LIMIT';if(m.includes('symbol')&&(m.includes('not found')||m.includes('invalid')||m.includes('does not exist')||m.includes('no data')))return'SYMBOL_NOT_FOUND';if(status===404&&m.includes('symbol'))return'SYMBOL_NOT_FOUND';if(status>=500)return'PROVIDER';return'REQUEST';}
async function fetchProviderJson(url,label){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),10_000);try{const response=await fetch(url,{signal:controller.signal,headers:{accept:'application/json'}}),text=await response.text();let payload=null;try{payload=text?JSON.parse(text):null;}catch{payload=null;}if(!response.ok){const message=String(payload?.message||payload?.detail||text||'request failed').slice(0,220),kind=classifyTwelveDataFailure(response.status,message),error=new Error(`${label} [${kind}] HTTP ${response.status}${message?`: ${message}`:''}`);error.status=response.status;error.provider='twelve-data';error.kind=kind;throw error;}return payload;}catch(error){if(error?.name==='AbortError'){const timeout=new Error(`${label} [NETWORK] request timed out.`);timeout.provider='twelve-data';timeout.kind='NETWORK';throw timeout;}throw error;}finally{clearTimeout(timer);}}
