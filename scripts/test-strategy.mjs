...
function analysis(overrides={}){
  const base={
    ...
    engines:{trend:engine(),entry:engine(),probability:engine(),riskReward:engine()},intradayConfirmation:confirmation(true)
  };
  return {...base,...overrides,engines:{...base.engines,...(overrides.engines||{})}};
}

assert.equal(evaluateStrategy(analysis(),null).state,'BUY WINDOW','Strong higher-timeframe setup must also have live participation confirmation to open a buy window.');
assert.equal(evaluateStrategy(analysis({intradayConfirmation:null}),null).state,'BUY CANDIDATE','A strong setup without participation confirmation must not open the buy window.');
assert.ok(evaluateStrategy(analysis({intradayConfirmation:confirmation(false)}),null).buyBlockers.includes('Participation / execution confirmation'),'Failed participation must be visible as a buy blocker.');
