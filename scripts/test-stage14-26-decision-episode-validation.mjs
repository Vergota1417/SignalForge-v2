...
assert.equal(episodes[0].episodeObservations,4,'four 15-minute BUY checks should be one BUY episode');
...
assert.equal(episodes.filter(x=>x.status==='BUY NOW').length,4,'BUY->READY->BUY, long-gap BUY, and new-model BUY should be separate decision episodes');

const evaluation=evaluateEvidenceRows(rows,{horizon:10,minSample:5});
assert.equal(evaluation.rawObservationSampleSize,8);
assert.equal(evaluation.sampleSize,5,'core evidence sample size must be decision episodes');
assert.equal(evaluation.collapsedObservationCount,3);
assert.equal(evaluation.calibration.buySampleSize,4,'four repeated 15-minute checks must not inflate BUY calibration');
assert.equal(evaluation.calibration.eligible,false,'repeated checks must not reach a five-episode calibration threshold');
assert.match(evaluation.calibration.reason,/BUY episodes/);
...
assert.equal(buySetup.sampleSize,4,'optimizer leaderboard must use distinct decision episodes');
assert.equal(buySetup.qualified,false,'four BUY episodes must not qualify at a five-episode threshold');
...
assert.equal(repeatedComparison.checks.sample,false,'30 repeated checks of one BUY thesis must not satisfy a 30-episode challenger threshold');
...
assert.equal(independentComparison.checks.sample,true,'30 distinct ticker episodes may satisfy the sample-size check');
...
assert.match(build,/version:'2\.30\.28'/);
assert.match(build,/shell:'v30-28'/);
assert.match(sw,/signalforge-shell-v30-28/);
assert.equal(pkg.version,'2.30.28');
