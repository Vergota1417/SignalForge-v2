...
assert.match(policy,/\/api\/provider-health/,'provider health polling must use the central request policy when its UI is reintroduced');
assert.match(ui,/API \/ Data Health/,'provider-health implementation must visibly identify the API health panel');
assert.match(ui,/Requests today/,'provider-health implementation must show per-provider request counts');
assert.match(ui,/Success \/ errors/,'provider-health implementation must retain success/error visibility');
assert.match(ui,/Last success/,'provider-health implementation must show the last successful upstream contact');
...
assert.ok(Number(shell[1])>=41,'PWA shell must never regress below v30-41');
