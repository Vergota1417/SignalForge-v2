...
let extended=refreshExecutionAnalysis(base(),confirmation({price:109}));assert.equal(extended.status,'WAIT FOR PULLBACK','Current execution price above overextension must not remain BUY NOW.');
let broken=refreshExecutionAnalysis(base(),confirmation({price:94}));assert.equal(broken.status,'SELL / EXIT','A current price below the thesis break must invalidate the setup.');
...
const ordinaryWait=base({dailyGatesReady:false,status:'WAIT — SETUP NOT READY',engines:{probability:engine(false),riskReward:engine(false)}});assert.equal(refreshIntervalFor({status:ordinaryWait.status,analysis:ordinaryWait}),4*60*60*1000,'A genuine 2/4 ordinary WAIT candidate should keep the lower-cost 4-hour deep refresh.');
...
const persistent=selectPromotionCandidates([quietQuote],[{symbol:'TEST',status:'SETUP — READY SOON',updatedAt:now-16*60*1000,analysis:base()}],{now,limit:1});assert.equal(persistent[0]?.symbol,'TEST','A READY SOON candidate must remain in the execution loop even after broad discovery activity cools.');
