(() => {
  ...
  function renderValidation(){const card=ensurePanel(),node=card?.querySelector('[data-pattern-validation]');if(!node)return;const health=patternHealth,primary=patternContext?.primaryPattern;if(!health){node.textContent='10-session shadow validation · collecting independent pattern outcomes · cannot affect BUY NOW.';return;}const required=Number(health.minSample)||20,horizon=Number(health.horizon)||10;if(primary){const key=`${primary.type} · ${primary.state||'UNKNOWN'}`,row=(health.patternSegments?.patternState||[]).find(item=>item.key===key);if(row){const need=Math.max(0,required-Number(row.sampleSize||0));if(need>0){node.innerHTML=`<strong>${escapeHtml(primary.type)} · ${escapeHtml(primary.state)}</strong> · ${row.sampleSize}/${required} independent ${horizon}-session outcomes · collecting ${need} more · shadow-only.`;return;}const candidate=(health.reviewCandidates||[]).some(item=>item.key===key);node.innerHTML=`<strong>${escapeHtml(primary.type)} · ${escapeHtml(primary.state)}</strong> · ${row.sampleSize} outcomes · win ${pct(row.winRate)} · expectancy ${signedPct(row.expectancy)} · market excess ${signedPct(row.avgMarketExcessReturn)} · ${candidate?'review candidate':'validating'} · still shadow-only.`;return;}}
    node.textContent=`${horizon}-session shadow validation · ${Number(health.sampleSize)||0}/${required} independent specialized-pattern observations resolved · ${String(health.status||'COLLECTING').toLowerCase()} · cannot affect BUY NOW.`;}
