(() => {
  ...
  let prefs=loadPrefs(),currentSymbol='',patternContext=null,currentMarket=null,timer=null;
  ...
  async function refresh(){
    ...
    try{const r=await fetch(`${API}/api/signals`,{headers:{accept:'application/json'},cache:'no-store'}),b=await r.json();if(!r.ok)throw new Error(b.error||`HTTP ${r.status}`);if(currentSymbol!==symbol)return;const row=(b.signals||[]).find(x=>String(x.symbol||'').toUpperCase()===symbol);if(row?.analysis?.patternContext)patternContext=row.analysis.patternContext;renderPanel();applyOverlays();}catch(error){renderUnavailable(String(error?.message||'Pattern context unavailable.'));}
  }
  ...
  function renderPanel(){const card=ensurePanel();if(!card)return;const p=patternContext;if(!p){renderUnavailable('Pattern Context will populate on the next fresh daily/deep analysis for this ticker.');return;}
    ...
    const families=new Map((p.patterns||[]).map(x=>[x.family,x]));card.querySelectorAll('[data-toggle]').forEach(btn=>{const key=btn.dataset.toggle,on=key==='channel'?channelEnabled(p):Boolean(prefs[key]);btn.classList.toggle('on',on);const family=key==='double'?'double':key==='triangles'?'triangles':key==='headShoulders'?'head-shoulders':key==='wedges'?'wedges':null,det=family?families.get(family):null;btn.classList.toggle('detected',Boolean(det&&det.state!=='CONFIRMED'));btn.classList.toggle('confirmed',Boolean(det?.state==='CONFIRMED'));if(key==='channel')btn.textContent=`Channel ${prefs.channel==='auto'?'Auto':on?'On':'Off'}`;});renderPatternList(card,p.patterns||[]);}
