(() => {
  ...
  style.textContent=`
    ...
    .sf-chart-actions{display:flex;gap:.4rem;align-items:center}.sf-chart-btn{border:1px solid #1b2d43;background:#0e1b2d;color:#c9d7e8;border-radius:8px;padding:.38rem .55rem;font-size:.7rem;cursor:pointer}.sf-chart-btn:active{transform:translateY(1px)}
    .sf-marker-note{display:flex;gap:.45rem;align-items:center;flex-wrap:wrap;margin:.15rem 0 .4rem;color:#72869f;font-size:.65rem}.sf-marker-dot{width:7px;height:7px;border-radius:50%;display:inline-block}.sf-marker-dot.hist{background:#7ebcff}.sf-marker-dot.private{background:#a887ff}
  ...
  let chart=null,candleSeries=null,volumeSeries=null,container=null,resizeObserver=null,currentKey='',currentPayload=null,levelLines=[],readout=null,markerApi=null,markersEnabled=true,currentBars=[],markerStatus=null,markerRefreshKey='';
  ...
  function ensureChart(){
    ...
    const toolbar=document.createElement('div');toolbar.className='sf-chart-toolbar';toolbar.innerHTML=`<div id="sfChartReadout" class="sf-chart-readout" aria-live="polite">Move across the chart to inspect OHLCV.</div><div class="sf-chart-actions"><button type="button" id="sfChartMarkers" class="sf-chart-btn">Markers on</button><button type="button" id="sfChartLatest" class="sf-chart-btn">Latest</button><button type="button" id="sfChartReset" class="sf-chart-btn">Reset view</button></div>`;wrap.appendChild(toolbar);readout=toolbar.querySelector('#sfChartReadout');
    ...
    chart.subscribeCrosshairMove(onCrosshairMove);toolbar.querySelector('#sfChartReset').addEventListener('click',()=>chart.timeScale().fitContent());toolbar.querySelector('#sfChartLatest').addEventListener('click',()=>chart.timeScale().scrollToRealTime());toolbar.querySelector('#sfChartMarkers').addEventListener('click',event=>{markersEnabled=!markersEnabled;event.currentTarget.textContent=markersEnabled?'Markers on':'Markers off';applyMarkers(markersEnabled?window.__sfDecisionMarkers||[]:[]);});
  ...
  function render(payload){
    ...
    currentBars=candles;candleSeries.setData(candles);volumeSeries.setData(volumes);currentPayload=payload;renderLevels(payload.analysis);renderLatestReadout();
    if(currentKey!==key){chart.timeScale().fitContent();currentKey=key;markerRefreshKey='';}
  ...
  async function refreshDecisionMarkers(symbol){
    ...
    window.__sfDecisionMarkers=dedupeMarkers(markers).slice(-30);applyMarkers(markersEnabled?window.__sfDecisionMarkers:[]);
    if(markerStatus)markerStatus.dataset.count=String(window.__sfDecisionMarkers.length);
  }
  async function fetchPublicHistory(symbol){
    const response=await priorFetch(`${API}/api/alerts?limit=50`,{headers:{accept:'application/json'}});if(!response.ok)return[];const body=await response.json(),rows=(body.alerts||[]).filter(r=>String(r.symbol).toUpperCase()===symbol);return rows.map(alertMarker).filter(Boolean);
  }
  ...
  async function fetchPrivateMarkers(symbol){
    try{
      const headers=await privateAuthHeaders(),[pRes,sRes]=await Promise.all([priorFetch(`${API}/api/portfolio`,{headers:{accept:'application/json',...headers}}),priorFetch(`${API}/api/strategy`,{headers:{accept:'application/json',...headers}})]);if(!pRes.ok||!sRes.ok)return[];
      const portfolio=await pRes.json(),strategy=await sRes.json(),out=[],position=(portfolio.positions||[]).find(r=>String(r.symbol).toUpperCase()===symbol),candidate=(strategy.ranked||[]).find(r=>String(r.symbol).toUpperCase()===symbol);
      if(position?.boughtAt)out.push({at:Number(position.boughtAt),text:'BOUGHT',color:'#a887ff',shape:'arrowUp',position:'belowBar'});
      if(candidate?.updatedAt&&(candidate.strategy?.state==='BUY WINDOW'||candidate.strategy?.state==='BUY CANDIDATE'))out.push({at:Number(candidate.updatedAt),text:candidate.strategy.state,color:'#2fd18b',shape:'arrowUp',position:'belowBar'});
      if(position?.strategy?.state&&currentPayload?.candles?.length){const latest=Number(currentPayload.candles.at(-1).time),state=String(position.strategy.state),style=portfolioStateStyle(state);out.push({at:latest,text:`CURRENT ${state}`,...style});}
      return out;
    }catch{return[];}
  }
  ...
  async function hydrateCurrent(){const {symbol,timeframe}=currentRequest();try{const response=await priorFetch(`${API}/api/market-data?symbol=${encodeURIComponent(symbol)}&timeframe=${encodeURIComponent(timeframe)}`,{headers:{accept:'application/json'}});if(!response.ok)return;render(await response.json());}catch(error){console.warn('[SignalForge chart] Lightweight hydration failed; Canvas fallback remains available.',error);}}
  window.addEventListener('signalforge:market-data',()=>{if(currentPayload)render(currentPayload);});document.addEventListener('click',event=>{if(event.target.closest('.timeframe-btn,.watch-item,.recent-item,.symbol-suggestion,#loadSymbolBtn,.radar-item,.alert-history-row'))setTimeout(hydrateCurrent,220);});window.addEventListener('load',()=>setTimeout(hydrateCurrent,0));if(document.readyState==='complete')setTimeout(hydrateCurrent,0);
})();
