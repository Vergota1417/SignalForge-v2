(() => {
  ...
  async function refreshRadar(){
    ...
    try{
      ...
      root.innerHTML=rows.length?rows.map((r,i)=>{const signal=signals.get(r.symbol),score=Number(r.rollingDiscoveryScore??r.discoveryScore??r.score)||0,velocity=Number(r.scoreVelocity)||0,eta=etaLine(r.expectation,signal);return`<button type="button" class="radar-item" data-symbol="${r.symbol}"><span class="radar-rank">#${i+1}</span><span class="radar-main"><strong>${r.symbol}</strong><small>${r.name||r.symbol}</small><span class="radar-decision ${statusClass(signal?.status)}">${statusShort(signal?.status)}</span>${signal?`<span class="radar-scan-time">Last deep scan ${fmtTime(signal.updatedAt)}</span>`:''}<span class="radar-eta ${eta.due?'eta-due':''}"><strong>${eta.label}</strong> ${eta.text}</span></span><span class="radar-stats"><b>${Math.round(score)}</b><small>Discovery ${velocity>=0?'↑':'↓'} ${Math.abs(velocity).toFixed(1)} · ${fmtPct(r.changePct)} · RV ${fmtVol(r.relativeVolume)}</small></span></button>`;}).join(''):'<div class="radar-empty">Radar is warming up. Dynamic discovery fills this list during market hours.</div>';
      ...
      const coverage=document.getElementById('radarCoverage');if(coverage)coverage.textContent=`Weekly pool ${radar.universeSize||0} · catalog ${radar.catalogSize||0} · ${radar.scannedSymbols||0} symbols have discovery history. ETAs show when SignalForge expects to check; the market determines whether the state changes.`;
  ...
  function escShort(v){return String(v||'Local').replace(/[<>]/g,'');}
