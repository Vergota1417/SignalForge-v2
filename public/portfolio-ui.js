(() => {
  ...
  btn.addEventListener('click',()=>open());q('[data-close]').addEventListener('click',close);q('[data-save]').addEventListener('click',save);

  async function authHeaders(){
    const token=localStorage.getItem(TOKEN_KEY)||'';
    if(!/^[A-Za-z0-9_-]{32,128}$/.test(token))throw new Error('Enable phone alerts first so SignalForge can authorize private portfolio access.');
    if(!('serviceWorker'in navigator))throw new Error('Portfolio access requires the installed SignalForge app or a browser with service workers.');
    const reg=await navigator.serviceWorker.ready, sub=await reg.pushManager?.getSubscription();
    if(!sub?.endpoint)throw new Error('Enable phone alerts first so SignalForge can authorize private portfolio access.');
  ...
  async function refresh(){try{const [portfolio,strategy]=await Promise.all([api('/api/portfolio'),api('/api/strategy')]);renderPositions(portfolio.positions||[]);renderRanked(strategy.ranked||[]);}catch(e){q('[data-list]').innerHTML=`<div class="muted portfolio-empty">${esc(e.message)}</div>`;q('[data-ranked]').innerHTML='';q('[data-summary]').innerHTML='<strong>Private portfolio locked</strong><span>Authorization is tied to your enabled SignalForge phone alerts.</span>';}}
